...
import ch.uzh.ifi.seal.changedistiller.distilling.refactoring.ClassRefactoringHelper;
import ch.uzh.ifi.seal.changedistiller.distilling.refactoring.FieldRefactoringHelper;
...
import ch.uzh.ifi.seal.changedistiller.distilling.refactoring.RefactoringExtractor;
import ch.uzh.ifi.seal.changedistiller.distilling.refactoring.RefactoringPair;
...
/**
 * Processes {@link RefactoringCandidate}s from a
 * {@link RefactoringCandidateContainer} and adds resulting changes to the
 * {@link ClassHistory}, and its member histories respectively.
 * 
 * @author Beat Fluri
 * @author Giacomo Ghezzi
 */
public class GitRefRefactoringCandidateProcessor {
	...
	@Inject
	GitRefRefactoringCandidateProcessor(DistillerFactory distillerFactory,
	...
	/**
	 * Processes all {@link RefactoringCandidate}s in the
	 * {@link RefactoringCandidateContainer} and stores all resulting changes to
	 ...
	 *            to process
	 */
	public void processRefactoringCandidates(ClassHistory classHistory,
	...
	public void processRefactoringCandidates(ClassHistory classHistory,
			ASTHelper<StructureNode> leftHelper,
			ASTHelper<StructureNode> rightHelper,
			RefactoringCandidateContainer candidates, String version) {
		fVersion = version;
		fClassHistory = classHistory;
		fLeftASTHelper = leftHelper;
		fRightASTHelper = rightHelper;
	...
	private void processInnerClassesRefactoringCandidates(
			RefactoringCandidateContainer candidates) {
		checkRefactorings(candidates.getAddedInnerClasses(),
				candidates.getDeletedInnerClasses(),
				new ClassRefactoringHelper(fClassHistory, fRightASTHelper));
	}

	private void processFieldRefactoringCandidates(
			RefactoringCandidateContainer candidates) {
		checkRefactorings(candidates.getAddedFields(),
				candidates.getDeletedFields(), new FieldRefactoringHelper(
						fClassHistory, fRightASTHelper));
	}

	private void processMethodRefactoringCandidates(
			RefactoringCandidateContainer candidates) {
		checkRefactorings(candidates.getAddedMethods(),
				candidates.getDeletedMethods(), new MethodRefactoringHelper(
						fClassHistory, fRightASTHelper));
	}

	private void checkRefactorings(List<RefactoringCandidate> added,
	...
	private StructureEntityVersion getCurrentClass() {
		return fClassHistory.getVersions().get(
				fClassHistory.getVersions().size() - 1);
	}

	@SuppressWarnings("unused")
	private void processRefactorings(
			AbstractRefactoringHelper refactoringHelper,
			List<RefactoringCandidate> added, List<RefactoringCandidate> deleted) {
		List<RefactoringPair> refactorings = RefactoringExtractor
				.extractRefactorings(added, deleted, refactoringHelper);
		for (RefactoringPair pair : refactorings) {
			StructureNode leftDrn = pair.getDeletedEntity().getDiffNode()
					.getLeft();
			StructureNode rightDrn = pair.getInsertedEntity().getDiffNode()
					.getRight();
			...
			if (fVersion != null) {
				...
				structureEntityVersion = refactoringHelper
						.createStructureEntityVersion(rightDrn);
			}

			Node rightRoot = fRightASTHelper.createDeclarationTree(rightDrn);
			// use the new qualified name for the method; otherwise
			// TreeDifferencer detects a return type change
			Node leftRoot = fLeftASTHelper.createDeclarationTree(leftDrn,
					rightRoot.getValue());
			if (isRenaming(leftDrn, rightDrn, refactoringHelper)) {
				if (fVersion != null) {
					structureEntityVersion = refactoringHelper
							.createStructureEntityVersionWithID(leftDrn,
									rightDrn.getFullyQualifiedName(), fVersion);
				} else {
					structureEntityVersion = refactoringHelper
							.createStructureEntityVersion(leftDrn,
									rightDrn.getFullyQualifiedName());
				}
				Update upd = new Update(structureEntityVersion,
						fLeftASTHelper.createSourceCodeEntity(leftDrn),
						fRightASTHelper.createSourceCodeEntity(rightDrn),
						leftRoot.getEntity());
				structureEntityVersion
						.addAllSourceCodeChanges(fChangeClassifier
								.classifySourceCodeChanges(Arrays.asList(upd)));
			}
	...
	private boolean isRenaming(StructureNode leftDrn, StructureNode rightDrn,
			AbstractRefactoringHelper refactoringHelper) {
	...
	private void processRemainingDiffs(StructureEntityVersion clazz,
		...
		for (RefactoringCandidate candidate : candidates) {
			if (!candidate.isRefactoring()) {
				List<SourceCodeChange> classifiedChanges = fChangeClassifier
						.classifySourceCodeChanges(Arrays.asList(candidate
								.getSourceCodeChange()));
