...
/**
 * Modified based on gitblit.com source code at:
...
public class PathModel implements Serializable, Comparable<PathModel> {
	...
	public PathModel(String _name, String _path, long _size, int _mode,
			String _objectId, String _commitId) {
		this.name = _name;
		this.path = _path;
		this.size = _size;
		this.mode = _mode;
	...
	public boolean isFile() {
		return FileMode.REGULAR_FILE.equals(mode)
				...
				|| (FileMode.MISSING.equals(mode) && !isSymlink()
						&& !isSubmodule() && !isTree());
	}
	...
	public static class PathChangeModel extends PathModel {
		...
		public PathChangeModel(String name, String path, long size, int mode,
		...
		public static PathChangeModel from(DiffEntry diff, String commitId) {
			...
			if (diff.getChangeType().equals(ChangeType.DELETE)) {
				pcm = new PathChangeModel(diff.getOldPath(), diff.getOldPath(),
						0, diff.getNewMode().getBits(), diff.getOldId().name(),
						commitId, diff.getChangeType());
			} else if (diff.getChangeType().equals(ChangeType.RENAME)) {
				pcm = new PathChangeModel(diff.getOldPath(), diff.getNewPath(),
						0, diff.getNewMode().getBits(), diff.getNewId().name(),
						commitId, diff.getChangeType());
			} else {
				pcm = new PathChangeModel(diff.getNewPath(), diff.getNewPath(),
						0, diff.getNewMode().getBits(), diff.getNewId().name(),
						commitId, diff.getChangeType());
			}
			return pcm;
